...
# (width_key, width_factor, length_key, panels)
_YIELD_FACTORS: dict[str, tuple[str, float, str, float]] = {
    "shirt":    ("bust",  0.55, "length", 2.0),
    "jacket":   ("bust",  0.60, "length", 2.5),
    "dress":    ("hip",   0.55, "length", 2.0),
    "skirt":    ("hip",   0.55, "length", 2.0),
    "trousers": ("hip",   0.55, "length", 2.2),
}

_WASTE_FACTOR = 1.15  # 15% added for seam allowances and cutting waste

# Candidate output garment types for Remake Mode (PRD scope: skirt + trouser)
_CANDIDATES = ["skirt", "trousers"]
...
def _required_area(garment_type: str, body: BodyProfile) -> tuple[float, str]:
    ...
    if garment_type == "skirt":
        ...
        required = (body.hip + 10) * min_length * _WASTE_FACTOR
    ...
    if garment_type == "trousers":
        ...
        required = (body.hip + 10) * (inseam + rise) * _WASTE_FACTOR
